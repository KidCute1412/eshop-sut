...
- **BrowserStack/LambdaTest:** not yet used — see Task 3 report for what remains to be done manually with a real trial.
...
- The 7 usability-test participants and their contact details — currently blank; must be real people the student recruits.
- Any SUS/UEQ-S scores, session quotes, or observation notes — currently blank templates.
- Cross-browser/cross-platform screenshots — currently a template with no images; must come from a real BrowserStack/LambdaTest trial or physical device.
- The actual GitHub Issues (only issue *content*, ready to paste, was AI-drafted; the act of creating them on a real repo is a manual step for the student).
