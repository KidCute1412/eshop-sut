...
- `<Thêm log/screenshot network timeout>`
...
Các test case có trạng thái `Passed`, `Not Executed` hoặc `Blocked` không được dùng để kết luận bug chức năng, trừ blocking issue `FR-03M` đã nêu rõ nguyên nhân là network timeout.
