...
|**#**<br>1|**Endpoint**<br>GET /api/products|**Mô tả & Lưu ý**<br>Lấy danh sách sản phẩm. Có hỗ trợ query string<br>?search=keyword .|
|---|---|---|
...
|5|DELETE<br>/api/products/:id|Xóa sản phẩm theo ID.|
...
|10|POST /api/forgot-<br>password|Yêu cầu đặt lại mật khẩu.|
|11|POST /api/reset-<br>assword|Đặt lại mật khẩu bằng token.|
||p||
...
|16|POST /api/checkout|Thanh toán giỏ hàng. Yêu cầu Auth Token. Tạo đơn hàng mới với trạng thái<br>pending .|
|17|GET /api/orders/my-<br>orders|Xem danh sách đơn hàng của mình. Yêu cầu Auth Token.|
|18|GET /api/orders/:id|Xem chi tiết đơn hàng theo ID.|
|19|PUT<br>/api/orders/:id/cancel|Hủy đơn hàng. Yêu cầu Auth Token. Có ràng buộc trạng thái (state transition).|
|20|POST /api/apply-coupon|Áp dụng mã giảm giá. Cần test mã hợp lệ/hết hạn/không tồn tại/đã dùng hết lượt.|
|21|PUT<br>/api/admin/orders/:id/st<br>atus|Admin cập nhật trạng thái đơn hàng. Có state transition:<br>pending→confirmed<br>→shipping→delivered .|
...
cd backend npm install npm install � global newman 
...
Tên tệp<br><!-- End of picture text -->



<!-- Start of picture text -->
Mô tả<br><!-- End of picture text -->



<!-- Start of picture text -->
Ghi chú<br><!-- End of picture text -->

|<ten-<br>api>.postman_collecti|Postman Collection|Chứa kịch bản kiểm thử của API đã chọn (bao gồm các test<br>assertions).|
|---|---|---|
|on.json|||
|local.postman_enviro|Cấu hình môi trường|Cấu hình biến<br>baseUrl(mặc định|
|nment.json||http)localhost:3000 ) và<br>studentId= <MSSV> .|
|miniM�api-|Tệp dữ liệu kiểm thử|Chứa ít nhất 5 test case. Ví dụ:<br>mini-login.data.json ,|
|name>.data.json|(Data-driven)|mini-cart.data.json .|
|newman-api-test.yml|Tệp cấu hình GitHub|Dùng để kích hoạt chạy kiểm thử tự động bằng Newman khi|
...
<!-- Start of picture text -->
Nhãn<br><!-- End of picture text -->



<!-- Start of picture text -->
Nhận xét hoặc chỉnh sửa<br>...<br><!-- End of picture text -->

|AI-01|VALID ,<br>INVALIDhoặc<br>INCOMPLETE|...|
|---|---|---|
|AI-02|...|...|
|...|...|...|
...
<!-- Start of picture text -->
curl http) localhost:3000/api/products/1<br><!-- End of picture text -->
...
Tạo tệp dữ liệu chạy thử đặt tên theo dạng miniM�api-name>.data.json (ví dụ mini-login.data.json , 

mini-cart.data.json ), điền các giá trị kiểm thử tương ứng. Đảm bảo cấu trúc JSON trùng khớp với các kịch bản kiểm thử và các biến mà bạn sẽ sử dụng trong Postman Collection. 
...
pm.test("[MINI] Response is JSON", () �� { pm.expect(pm.response.headers.get("Content-Type")).to.include( "application/json", ); }); 
...
miniM�api-name>.postman_collection.json mini-local.postman_environment.json 
...
newman run miniM�api-name>.postman_collection.json \ 

� environment mini-local.postman_environment.json \ 

� iteration-data miniM�api-name>.data.json \ 
...
Commit và push bài làm lên nhánh riêng của bạn (ví dụ feature/<MSSV> ) trên repository nhóm đã fork từ 

eshop-sut . Mở tab **Actions** trên GitHub, chọn đúng nhánh của bạn và chờ workflow Newman API tests chạy hoàn thành. 
...
<!-- Start of picture text -->
Feature<br><!-- End of picture text -->



<!-- Start of picture text -->
Đã dùng?<br><!-- End of picture text -->
...
2. miniM�api-name>.data.json — tệp dữ liệu kiểm thử. 

3. miniM�api-name>.postman_collection.json và mini-local.postman_environment.json . 
...
- Postman Data-driven Testing Guide (sử dụng iteration data files): https://learning.postman.com/docs/sendingrequests/runner/running-multiple-iterations/ 

- Newman Command-line Integration Guide: https://learning.postman.com/docs/collections/using-newmancli/command-line-integration-with-newman/ 
