...
# API Description and Verified Contract

## API Under Test

- Method: GET
- Path: /api/products
- Purpose: Retrieve product list, optionally filtered by product name.
- Authentication: Not required.
- Query parameter: `search` is optional and is used in a SQL LIKE filter against product name.
- Success response: HTTP 200 with JSON array of products.
- Product fields: `id`, `name`, `price`, `description`, `imageUrl`, `category_id`.
- Empty or missing search: returns all seeded products.
- Non-matching search: returns an empty array.
- Source files inspected: `backend/server.js`, `backend/database.sqlite`.

## Implementation Risks Recorded

- SQL injection is possible because `search` is interpolated into the SQL query.
- LIKE wildcard characters `%` and `_` are treated as SQL wildcards.
- Database errors can return HTML instead of JSON.

---

# Step 5 - Postman Features

| Feature | Used? | Notes |
|---|---|---|
| Collections | Yes | Used `mini-products.postman_collection.json` to define the API request and assertions. |
| Environment variables | Yes | Used `baseUrl` and `studentId` in `mini-local.postman_environment.json`. |
| Collection variables | Yes | Used collection variable `search` as the default query value. |
| Pre-request scripts | Yes | Injected `X-Student-Id` from the environment before each request. |
| Test scripts (assertions) | Yes | Asserted status, JSON content type, product count, and response time. |
| Data-driven runs | Yes | Used `mini-products.data.json` with exactly 5 iterations. |
| Newman CLI | Yes | Executed the collection and exported `mini-newman-report.json`. |
| Monitors | No | Not used because the exercise only requires local and CI execution. |
| Mock servers | No | Not used because tests target the real local backend provider. |
| Workspaces | No | Not required for the submitted local collection files. |
