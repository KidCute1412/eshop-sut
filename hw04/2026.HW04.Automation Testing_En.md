...
- AI First strategy. You are required to apply AI to the testing techniques covered in class. However, this does not mean issuing a single, generic prompt such as "write allthe automation scripts for this feature." Instead, you must guide the AI through every step of the technique as it was taught, using the AI as a disciplined assistant rather than a black box. 
...
- Critically review the AI generated automation: fix it, analyse what the AI got wrong or missed, and produce complete outputs LHTML reports and bug reports). 

- 

- Demonstrate Bloom AI competencies at levels G9.2 LApplyM, G9.3 

- LAnalyse), and G9.4 LCollaborate with AIM. 
...
- Automate the same three L3) web features you selected in HW02 — one each from Pools A, B, and C. The Pool D (mobile) feature is not used in this homework, because here you automate the web frontend. 
...
- Pools A C directly in this report and clearly state the reason HW02 is unavailable. 
...
- Make the scripts data-driven. The test data must be stored in a separate **`.csv`** or **`.json`** file (hardcoded inline arrays or objects in the script are not 

- accepted), and the scripts must use at least three distinct assertion patterns. 

- Run on at least 3 browsers LChromium / Firefox / WebKit, or Chrome / Edge / Firefox). Each feature must run on all three browsers — at least 9 browser runs in total across the suite. Each run must produce an HTML report LAllure or the Playwright HTML reporter) that visibly displays "Run by: PStudentIDQ" (in the title, header, footer, or report metadata). 
...
- Record an unlisted YouTube video of at least 5 minutes, narrated in Vietnamese, demonstrating one of your automation scripts running end to - 
...
- Submit the skill together with a demonstration video LYouTube link) that shows, end to end, how you used the skill on a complete feature. 
...
- The required Bloom AI level for this homework is G9.2 LApplyM, G9.3 LAnalyse), and G9.4 LCollaborate). 
...
- The HTML reports, which must contain "Run by: PStudentIDQ" together with an ISO timestamp. 
...
- Maintain a public GitHub repository with a meaningful history: at least 8 - 

- commits over at least 4 days. Only commits that change test script files ( `.spec.js` , `.spec.ts` , or equivalent) count toward the 8-commit minimum; - 
...
##### Required contents of the **`.zip`** : 
   ...
   - Main report LMarkdown + PDFM, including the automation report and - 
   ...
   - The multi browser HTML reports LAllure / Playwright). 
...
|---|---|---|---|
|1|Task1 ]Feature A|25||
|1|Task1 ]Feature B|25||
|1|Task1 ]Feature C|25||
|2|Task2 _Demo video|15||
|3|Agent Skills|10||
