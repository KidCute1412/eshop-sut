...
def build_postman_collection(name, endpoint, method, test_cases, student_id="23127404"):
    """Synthesizes an executable Postman collection and its authentication bootstrap."""
    rendered_endpoint = endpoint.replace(":id", "{{orderId}}")
    auth_bootstrap_items = []
    if endpoint in {"/api/checkout", "/api/admin/orders/:id/status"}:
        for role, email, password, token_key in (
            ("admin", "admin@eshop.com", "Admin123!", "adminToken"),
            ("user", "test@eshop.com", "Test1234!", "userToken"),
        ):
            auth_bootstrap_items.append({
                "name": f"00 Bootstrap {role} token",
                "event": [{
                    "listen": "prerequest",
                    ...
                    "listen": "test",
                    "script": {"type": "text/javascript", "exec": [
                        f"pm.test('bootstrap {role} login succeeds', () => pm.response.to.have.status(200));",
                        "const body = pm.response.json();",
                        f"pm.test('bootstrap {role} token is present', () => pm.expect(body.token).to.be.a('string').and.not.empty);",
                        f"pm.environment.set('{token_key}', body.token);"
                    ]}
                }],
                "request": {
                    "method": "POST",
                    "header": [{"key": "Content-Type", "value": "application/json"}],
                    "body": {"mode": "raw", "raw": json.dumps({"email": email, "password": password})},
    ...
    collection = {
        "info": {
            ...
            "description": f"AI-Driven Test Generator Suite for {endpoint}. Protected suites bootstrap valid local tokens before every data iteration; run the complete collection.",
            "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
        },
        "item": auth_bootstrap_items + [
            {
                ...
                "event": [
                    {
                        ...
                        "script": {
                            ...
                            "exec": [
                                ...
                                "  if (!token) throw new Error('Missing adminToken: run the complete generated collection so its bootstrap login can obtain it.');",
                                "  pm.request.headers.upsert({ key: 'Authorization', value: 'Bearer ' + token });",
                                "} else if (authRole === 'user' || authRole === 'valid_user') {",
                                "  const token = pm.environment.get('userToken');",
                                "  if (!token) throw new Error('Missing userToken: run the complete generated collection so its bootstrap login can obtain it.');",
                                "  pm.request.headers.upsert({ key: 'Authorization', value: 'Bearer ' + token });",
                                ...
                                "}",
                                "if (pm.iterationData.get('order_id') !== undefined) {",
                                "  pm.variables.set('orderId', String(pm.iterationData.get('order_id')));",
                                "}",
                ...
                "request": {
                    ...
                    "url": {
                        "raw": "{{baseUrl}}" + rendered_endpoint,
                        "host": ["{{baseUrl}}"],
                        "path": [p for p in rendered_endpoint.strip("/").split("/") if p]
                    }
                },
                "response": []
            }
