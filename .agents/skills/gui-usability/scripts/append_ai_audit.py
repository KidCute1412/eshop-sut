#!/usr/bin/env python3
"""Append, never overwrite, a validated AI audit entry for HW03 GUI & Usability."""
...
def main() -> int:
    ...
    parser.add_argument("--audit-file", required=True, help="Markdown file to create or append.")
    parser.add_argument("--ai-tool", required=True, type=non_empty)
    parser.add_argument("--task-id", required=True, help="Free-form task label, e.g. TASK1-CHECKLIST, TASK2-USABILITY, TASK3-CROSSPLATFORM, AGENT-SKILL.")
    parser.add_argument("--task", required=True, type=non_empty, help="Human-readable description of what the AI was asked to do.")
    ...
    parser.add_argument("--output-refs", required=True, help="Comma/semicolon/newline-separated output references.")
    ...
    if not TASK_ID_RE.fullmatch(task_id):
        print("ERROR: task-id must be alphanumeric with optional . _ - characters.", file=sys.stderr)
        return 2
    try:
        if args.prompt_file:
    ...
    try:
        ...
        with audit_file.open("a", encoding="utf-8", newline="") as handle:
            if new_file:
                handle.write("# AI Audit Report - HW03 GUI & Usability\n")
            handle.write(
