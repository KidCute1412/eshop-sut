#!/usr/bin/env python3
"""Append, never overwrite, a validated AI audit entry."""
...
def validate_output_refs(raw: str) -> list[str]:
    ...
    for ref in refs:
        # URLs and descriptive references are not local paths. Path-like values must exist.
...
def main() -> int:
    ...
    parser.add_argument("--task", required=True, type=non_empty)
    parser.add_argument("--feature-id", required=True, help="FR-01 or D-01.")
    ...
    if not FEATURE_ID_RE.fullmatch(feature_id):
        print("ERROR: feature-id must be FR-01 or D-01.", file=sys.stderr)
        return 2
    try:
        if args.prompt_file:
