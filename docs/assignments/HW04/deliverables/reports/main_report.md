...
| Execution revision | Pre-`aa316e0` local state; exact run revision not embedded, likely `238cf27` or `532ebc2` depending on run |
...
The deliverables suite defines 51 external-data cases: 16 for FR-06, 16 for FR-10, and 19 for FR-12. All cases were scheduled on Chromium, Firefox, and WebKit in nine selected feature/browser runs. The nine complete HTML/JSON/metadata report directories passed structural identity, timestamp, case-count, completion, and totals validation. Across 153 browser attempts, 131 reached assertions: 79 passed and 52 failed across 19 unique logical cases. The other 22 attempts failed during Firefox `browserContext.newPage()` fixture setup before the test body could observe the SUT.

FR-06 produced 25 passes and 23 recorded failures across 48 executions; seven failures were environmental and 16 were assertions across six cases. FR-10 produced 29 passes and 19 recorded failures across 48 executions; seven were environmental and 12 were assertions across four cases. FR-12 produced 25 passes and 32 recorded failures across 57 executions; eight were environmental and 24 were assertions across nine cases.
...
|---|---|---|---|---:|---|
| FR-06 | Source `FR06-01`–`FR06-16` | Existing products and valid quantity boundaries | Content/format, quantity constraints, feedback, unknown ID | 16 cases × 3 browsers | 48 attempted; 41 reached assertions: 25 pass, 16 fail; 7 fixture failures |
| FR-10 | Source `FR10-01`–`FR10-16` | Permitted admin/user transitions | Skipped transitions, role limits, and final-state exits | 16 cases × 3 browsers | 48 attempted; 41 reached assertions: 29 pass, 12 fail; 7 fixture failures |
| FR-12 | Source `FR12-01`–`FR12-19` | Authorized Admin UI/API operations | Guest/user/malformed authorization and protected mutations | 19 cases × 3 browsers | 57 attempted; 49 reached assertions: 25 pass, 24 fail; 8 fixture failures |
...
| Execution revision/data seed | Pre-`aa316e0` current local state; exact report revision is not embedded and was likely around `238cf27` or `532ebc2` depending on run; backend seed/reset state used by suite |
...
|---|---|---|---:|---:|---:|---:|---|
| `RUN-A-CHR` | FR-06 / Chromium | `2026-08-09T09:50:04.895Z` | 16 | 10 | 6 | 0 | `automation/reports/fr06-chromium-2026-08-09T09-50-04-895Z/` |
| `RUN-A-FF` | FR-06 / Firefox | `2026-08-09T10:07:59.582Z` | 16 | 5 | 11 | 7 | `automation/reports/fr06-firefox-2026-08-09T10-07-59-582Z/` |
| `RUN-A-WK` | FR-06 / WebKit | `2026-08-09T09:52:14.569Z` | 16 | 10 | 6 | 0 | `automation/reports/fr06-webkit-2026-08-09T09-52-14-569Z/` |
| `RUN-B-CHR` | FR-10 / Chromium | `2026-08-09T10:26:40.345Z` | 16 | 12 | 4 | 0 | `automation/reports/fr10-chromium-2026-08-09T10-26-40-345Z/` |
| `RUN-B-FF` | FR-10 / Firefox | `2026-08-09T10:27:32.954Z` | 16 | 5 | 11 | 7 | `automation/reports/fr10-firefox-2026-08-09T10-27-32-954Z/` |
| `RUN-B-WK` | FR-10 / WebKit | `2026-08-09T10:32:34.318Z` | 16 | 12 | 4 | 0 | `automation/reports/fr10-webkit-2026-08-09T10-32-34-318Z/` |
| `RUN-C-CHR` | FR-12 / Chromium | `2026-08-09T09:56:03.803Z` | 19 | 10 | 9 | 0 | `automation/reports/fr12-chromium-2026-08-09T09-56-03-803Z/` |
| `RUN-C-FF` | FR-12 / Firefox | `2026-08-09T10:18:54.806Z` | 19 | 5 | 14 | 8 | `automation/reports/fr12-firefox-2026-08-09T10-18-54-806Z/` |
| `RUN-C-WK` | FR-12 / WebKit | `2026-08-09T09:57:54.553Z` | 19 | 10 | 9 | 0 | `automation/reports/fr12-webkit-2026-08-09T09-57-54-553Z/` |
| **Total** | **3 features / 3 browsers** | — | **153 attempts** | **79** | **74** | **22** | **Nine structurally validated directories** |

The 74 runner failures equal 52 assertion failures plus 22 Firefox environment failures. All nine selected HTML/JSON/metadata report sets passed structural identity, timestamp, case-count, completion, report/data-directory presence, and totals validation. Exact shell invocations were not separately retained in this documentation task; the suite's matrix workflow is `npm run run:matrix`, and that fact must not be misrepresented as a verbatim per-run command record.

The selected reports predate `aa316e0`. Current source at `aa316e0` attaches the complete external case row to each newly generated result, but that enhancement is not retroactively present in these reports. Existing report titles and case IDs still map results to external data; do not claim the selected evidence was generated from `aa316e0`.
...
| Firefox environment | A browser failure could be misreported as a product defect | Headless Firefox failed before `newPage()`; headful runs still had intermittent page-setup timeouts | Firefox configured headful; 22 `newPage()` timeouts classified as environment failures | Final Firefox JSON reports: 7 FR-06, 7 FR-10, 8 FR-12 environment failures |
| FR-12 / authorization assertions | Passing over status mismatches would hide security behavior | FR-12 requires valid admin JWT and role for protected endpoints | Expected 401/403 assertions were retained; actual 200/400 responses remain failures | 24 assertion failures across nine logical cases in the selected reports |

The mandatory critique is in `ai_critique.md`. The interaction record is in `ai_audit_report.md`.

## 6. Defects and issue tracking

The final runs produced 52 assertion failures across 19 unique logical cases: six FR-06, four FR-10, and nine FR-12 cases. These are documented as runtime-observed candidates in `../bugs/bug_report.md`; the 22 Firefox `newPage()` failures are documented separately as environment failures. No candidate is represented as a filed GitHub defect because no verified public HW04 Issue URL or attached public screenshot has been supplied.
...
- Firefox produced 22 `newPage()` environment failures even in headful mode; those cases were not rerun successfully in Firefox.
...
| Attempts reaching assertions | 41 | 41 | 49 | **131** |
| Passed assertions | 25 | 29 | 25 | **79** |
| Recorded runner failures | 23 | 19 | 32 | **74** |
| Environment failures within recorded failures | 7 | 7 | 8 | **22** |
| Assertion failures | 16 | 12 | 24 | **52** |
| Unique assertion-failing logical cases | 6 | 4 | 9 | **19** |
| Verified public HW04 Issues | 0 | 0 | 0 | **0** |

The arithmetic reconciles: `79 + 52 + 22 = 153`, `79 + 52 = 131`, and `52 + 22 = 74`. Issue counts must be updated only from verified public URLs.
