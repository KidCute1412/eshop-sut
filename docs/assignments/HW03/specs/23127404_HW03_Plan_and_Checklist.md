...
| **Task 2** | **Usability Evaluation** (Kịch bản + 7 phiên thử nghiệm + Phân tích) | **40 điểm** | • Chọn 1 luồng end-to-end liên tục **thuộc 1 vai trò duy nhất (Khách hàng / Customer)**: **FR-23 (Mobile Product Detail) → FR-07 (Shopping Cart) → thao tác hỗ trợ để tạo đơn → FR-10 (Order State Machine) → FR-11 (View Order History)**. Thao tác tạo đơn chỉ là điều kiện để hoàn tất luồng; **không mô tả FR-10 là Checkout**.<br>• *Lưu ý:* FR-18 (Admin Order Management) thuộc vai trò Admin nên chỉ kiểm thử ở Task 1 (GUI Checklist), không đưa vào luồng Usability của khách hàng.<br>• **Quay màn hình 7 phiên chính thức**; nên quay thêm phiên pilot để có bằng chứng tinh chỉnh kịch bản (chỉ ghi audio khi participant đồng ý).<br>• Thực hiện **7 phiên thử nghiệm** với **7 người thật** (không thuộc lớp HW03, che 4 số giữa thông tin liên hệ).<br>• Agent tự động tính điểm SUS, nhóm pain points, phân loại severity và soạn bug report từ dữ liệu phiên thử nghiệm thật đã được human review. |
...
- **Ánh xạ nội bộ cho người quan sát (không đọc cho participant)**: FR-23 Product Detail → FR-07 Shopping Cart → thao tác hỗ trợ tạo đơn → FR-10 Order State Machine → FR-11 Order History.
...
- [ ] **Bước 3.1**: Định nghĩa mục tiêu đánh giá usability và soạn Kịch bản nhiệm vụ (Task Scenario) thuần túy vai trò Khách hàng (Customer):
  ...
  - *Ánh xạ nội bộ*: FR-23 → FR-07 → thao tác hỗ trợ tạo đơn → FR-10 → FR-11.
